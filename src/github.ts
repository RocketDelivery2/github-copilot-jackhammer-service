...
async function githubRequest<T>(label: string, operation: () => Promise<T>): Promise<T> {
  return retryTransient(operation, { label });
}

export async function doctorGithub(): Promise<void> {
  const repo = await githubRequest('repos.get', () =>
    octokit.repos.get({ owner: config.GITHUB_OWNER, repo: config.GITHUB_REPO }),
  );
...
export async function ensureLabels(): Promise<void> {
  for (const name of labels) {
    try {
      await githubRequest(`issues.getLabel:${name}`, () =>
        octokit.issues.getLabel({ owner: config.GITHUB_OWNER, repo: config.GITHUB_REPO, name }),
      );
    } catch (err: any) {
      if (err.status !== 404) throw err;
      await githubRequest(`issues.createLabel:${name}`, () =>
...
export async function createIssue(task: AiTask, hash: string, commitSha: string): Promise<{ number: number; url: string }> {
  ...
  const issue = await githubRequest('issues.create', () =>
...
export async function findExistingIssueByHash(hash: string): Promise<{ number: number; url: string } | null> {
  ...
  const res = await githubRequest('search.issuesAndPullRequests', () => octokit.search.issuesAndPullRequests({ q, per_page: 1 }));
...
export async function getIssue(issueNumber: number): Promise<IssueData> {
  const res = await githubRequest('issues.get', () =>
    octokit.issues.get({
      owner: config.GITHUB_OWNER,
      repo: config.GITHUB_REPO,
...
export async function getIssueComments(issueNumber: number): Promise<string[]> {
  const res = await githubRequest('issues.listComments', () =>
...
export async function findLinkedPRs(issueNumber: number): Promise<LinkedPR[]> {
  ...
  try {
    const res = await githubRequest('search.issuesAndPullRequests', () => octokit.search.issuesAndPullRequests({ q, per_page: 10 }));
    const prNumbers = res.data.items.map(i => i.number);
    const results: LinkedPR[] = [];
    for (const n of prNumbers) {
      try {
        const pr = await githubRequest('pulls.get', () =>
          octokit.pulls.get({ owner: config.GITHUB_OWNER, repo: config.GITHUB_REPO, pull_number: n }),
        );
        results.push({
...
export async function getPR(prNumber: number): Promise<PRData> {
  const res = await githubRequest('pulls.get', () =>
    octokit.pulls.get({
      owner: config.GITHUB_OWNER,
      repo: config.GITHUB_REPO,
...
export async function getPRReviews(prNumber: number): Promise<Array<{ state: string; body: string }>> {
  const res = await githubRequest('pulls.listReviews', () =>
...
export async function getPRChecks(prNumber: number): Promise<CheckRun[]> {
  ...
  const res = await githubRequest('checks.listForRef', () =>
...
export async function postComment(issueOrPRNumber: number, body: string): Promise<void> {
  ...
  await githubRequest('issues.createComment', () =>
...
export async function approvePR(prNumber: number): Promise<void> {
  ...
  await githubRequest('pulls.createReview', () =>
...
export async function mergePR(prNumber: number): Promise<void> {
  ...
  await githubRequest('pulls.merge', () =>
    octokit.pulls.merge({
      owner: config.GITHUB_OWNER,
      repo: config.GITHUB_REPO,
      pull_number: prNumber,
...
export async function closeIssue(issueNumber: number): Promise<void> {
  ...
  await githubRequest('issues.update', () =>
...
export async function deleteBranch(branchName: string): Promise<void> {
  ...
  try {
    await githubRequest('git.deleteRef', () =>
...
export async function resolveRepositoryNodeId(): Promise<string> {
  const response = await githubRequest('graphql.repository.id', () =>
    octokit.graphql<{ repository: { id: string } }>(
...
export async function resolveDiscussionCategoryBySlugOrName(slugOrName: string): Promise<DiscussionCategory | null> {
  const response = await githubRequest('graphql.discussionCategories', () =>
    octokit.graphql<{
      repository: {
...
export async function readRecentDiscussions(limit = 20): Promise<RecentDiscussion[]> {
  const response = await githubRequest('graphql.discussions', () =>
    octokit.graphql<{
      repository: {
        discussions: {
          nodes: Array<{
...
export async function createRepositoryDiscussion(input: {
  ...
  const response = await githubRequest('graphql.createDiscussion', () =>
    octokit.graphql<{
      createDiscussion: {
        discussion: {
          id: string;
...
export async function getLatestPublishedRelease(windowDays: number): Promise<RepositoryRelease | null> {
  ...
  const response = await githubRequest('repos.listReleases', () =>
...
export async function listMergedPullRequests(windowDays: number): Promise<RepositoryPullRequestActivity[]> {
  const since = toIsoDaysAgo(windowDays);
  const response = await githubRequest('pulls.list', () =>
    octokit.pulls.list({
      owner: config.GITHUB_OWNER,
      repo: config.GITHUB_REPO,
      state: 'closed',
...
export async function listClosedIssues(windowDays: number): Promise<RepositoryIssueActivity[]> {
  const since = toIsoDaysAgo(windowDays);
  const response = await githubRequest('issues.listForRepo:closed', () =>
...
export async function listRoadmapIssues(windowDays: number): Promise<RepositoryIssueActivity[]> {
  const since = toIsoDaysAgo(windowDays);
  const response = await githubRequest('issues.listForRepo:roadmap', () =>
    octokit.issues.listForRepo({
      owner: config.GITHUB_OWNER,
      repo: config.GITHUB_REPO,
      state: 'open',
...
export async function listRecentCommits(windowDays: number): Promise<RepositoryCommitActivity[]> {
  const response = await githubRequest('repos.listCommits', () =>
